...
- **Payment Provider**: Dodo Payments or PayPal

### Generated Files

After running `init`, you'll get:

<Tabs items={['Next.js + Dodo Payments', 'Express.js + Dodo Payments', 'React.js + Dodo Payments', 'Next.js + PayPal', 'Express.js + PayPal', 'React.js + PayPal']}>
...
For PayPal integrations, the following API routes are created:

- `POST /api/(paypal)/checkout` - Create PayPal order
- `GET /api/(paypal)/order/[orderId]` - Get order details
- `POST /api/(paypal)/order/capture` - Capture PayPal order
...
<Callout title="Environment Setup">
  After running `init`, copy the example environment file and configure your payment provider API keys and webhook secrets:
...
<Tabs items={['Dodo Payments', 'PayPal']}>
  <Tab value="Dodo Payments">
    ```javascript
    // app.js or server.js
  ...
  <Tab value="PayPal">
    ...
    // app.js or server.js
    const express = require('express');
    const { paypalRouter } = require('./src/routes/paypal/route');
...
<Tabs items={['Dodo Payments', 'PayPal']}>
  ...
  <Tab value="PayPal">
    ...
    function App() {
      const { createOrder, captureOrder, loading, error } = usePayPal()

      const handlePayment = async () => {
        try {
          // Create a PayPal order
          const orderId = await createOrder('100.00', 'USD', 'BillingSDK Purchase')
          
          // In a real app, you would redirect to PayPal for approval
          // Then capture the order after approval
...
<Tabs items={['Dodo Payments', 'PayPal']}>
  ...
  <Tab value="PayPal">
    ...
    VITE_PAYPAL_WEBHOOK_ID=your_webhook_id_here
